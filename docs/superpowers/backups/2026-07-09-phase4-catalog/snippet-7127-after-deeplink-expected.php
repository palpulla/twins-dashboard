 ...
 * Deep link: ?product={ProductId} auto-opens that collection's design step.
...
(function () {
	...
	loadList().then(function (list) {
		state.products = list;
		/* deep link: ?product={ProductId} -> straight to that collection's design step */
		var dm = /[?&]product=(\d+)/.exec(location.search);
		var di = -1;
		if (dm) {
			for (var i = 0; i < list.length; i++) {
				if (String(list[i].ProductId) === dm[1]) { di = i; break; }
			}
		}
		if (di >= 0) { pickProduct(di); } else { render(); }
	}).catch(function () {
		state.step = 'fallback';
		render();
	});
