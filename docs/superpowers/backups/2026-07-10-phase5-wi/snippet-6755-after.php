...
// ===== Phase 5: [clopay_collection_grid] for /wi (ported from main snippet 7050) =====
// Cards link primary -> /wi door-builder (?product={id}); secondary "View full
// collection" -> the collection page on the main site (absolute URL). Base card
// CSS (.twx2-grid/.twx2-gcard) already ships in the twx v2 style block above;
// only the split-link additions are emitted here. Reversible: remove this block.

// --- Residential products list: fetch + cache (24h transient, durable last-good fallback) ---
if ( ! function_exists( 'twins_clopay_get_products_list' ) ) {
...
// --- Product id -> MAIN-SITE collection page URL (slugs per Phase 4 content pack;
// filterable so later tasks can sync if the pack renames a slug) ---
function twins_wi_clopay_collection_url( $product_id, $title = '' ) {
	$map = array(
		8   => 'clopay-reserve-wood-custom',
		9   => 'clopay-reserve-wood-semi-custom',
		10  => 'clopay-reserve-wood-limited-edition',
		11  => 'clopay-coachman',
		12  => 'clopay-gallery-steel',
		13  => 'clopay-classic-collection',
		16  => 'clopay-avante',
	...
	return 'https://twinsgaragedoors.com/' . $slug . '/';
}

// --- [clopay_collection_grid] — all 23 residential collections, server-side ---
function twins_wi_clopay_collection_grid_shortcode() {
	...
	foreach ( $list as $p ) {
		...
		$builder    = add_query_arg( 'product', (int) $p['ProductId'], home_url( '/door-builder/' ) );
		$collection = twins_wi_clopay_collection_url( $p['ProductId'], $title_disp );
		echo '<div class="twx2-gcard">';
		echo '<a class="twx2-gcard-main" href="' . esc_url( $builder ) . '">';
		if ( $img ) {
		...
		echo '</span></a>';
		echo '<a class="twx2-gcard-alt" href="' . esc_url( $collection ) . '">View full collection &rarr;</a>';
		echo '</div>';
	}
	echo '</div>';
	return ob_get_clean();
}
if ( ! shortcode_exists( 'clopay_collection_grid' ) ) {
	add_shortcode( 'clopay_collection_grid', 'twins_wi_clopay_collection_grid_shortcode' );
}

// Split-link card additions (primary builder link + secondary collection link).
add_action( 'wp_head', function () {
	?>
<style id="twx2-gcard-wi">
.twx2-gcard-main,.twx2-gcard-main:visited{display:flex;flex-direction:column;flex:1;text-decoration:none!important;color:var(--tw-navy)!important}
.twx2-gcard-alt,.twx2-gcard-alt:visited{display:block;padding:10px 16px;border-top:1px solid #e7ecf2;font-weight:800;font-size:13px;color:var(--tw-navy)!important;text-decoration:none!important;background:#fff}
.twx2-gcard-alt:hover{background:var(--tw-soft)}
</style>
	<?php
} );
